<template>
    <div ref="item" class="grid" @click="handleClick" @pointerenter="hover = true" @pointerleave="hover = false">
        ...
        >
            <div v-show="hover" class="h-100% grid gap-y-2 col-[1/-1] row-[1/-1] relative z-0 will-change-transform">
                <div class="absolute top-0 left-0 w-100% h-35% -z-1 op-75" :class="[$style['gradient--top']]"></div>
                <div class="self-start flex justify-between gap-x-2 p-4">
                    <Button type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800" @click.native="handleCopy">
                        <span class="w-4 h-4 relative -bottom-[0.3ex] mr-2 -ml-1">
                            <svg class="block w-full h-full" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" width="32" height="32" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32"><path fill="currentColor" d="M11.947 19a4.948 4.948 0 0 1-3.499-8.447l5.106-5.104a4.948 4.948 0 0 1 6.998 6.998l-.553.552l-1.415-1.413l.557-.557a2.95 2.95 0 0 0-.004-4.166a3.02 3.02 0 0 0-4.17 0l-5.104 5.104a2.947 2.947 0 0 0 0 4.17a3.02 3.02 0 0 0 4.17 0l1.414 1.414a4.918 4.918 0 0 1-3.5 1.449Z"></path><path fill="currentColor" d="M19.947 17a4.948 4.948 0 0 1-3.499-8.447l.553-.552l1.414 1.415l-.552.552a2.948 2.948 0 0 0 0 4.169a3.02 3.02 0 0 0 4.17 0l5.105-5.105a2.951 2.951 0 0 0 0-4.168a3.02 3.02 0 0 0-4.17 0l-1.414-1.415a4.948 4.948 0 0 1 6.998 6.998l-5.104 5.103a4.92 4.92 0 0 1-3.5 1.45Z"></path><path fill="currentColor" d="M24 30H4a2.002 2.002 0 0 1-2-2V8a2.002 2.002 0 0 1 2-2h4v2H4v20h20V18h2v10a2.002 2.002 0 0 1-2 2Z"></path></svg>
                        </span>
                        Copy
                    </Button>
                    <Button type="button" class="text-white bg-orange-700 hover:bg-orange-800 focus:ring-orange-300 dark:bg-orange-600 dark:hover:bg-orange-700 dark:focus:ring-orange-800">
                        <span class="w-4 h-4 relative -bottom-[0.4ex] mr-2 -ml-1">
                            <svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" aria-hidden="true" role="img" class="block w-full h-full" width="32" height="32" preserveAspectRatio="xMidYMid meet" viewBox="0 0 32 32"><path fill="currentColor" d="M2 26h28v2H2zM25.4 9c.8-.8.8-2 0-2.8l-3.6-3.6c-.8-.8-2-.8-2.8 0l-15 15V24h6.4l15-15zm-5-5L24 7.6l-3 3L17.4 7l3-3zM6 22v-3.6l10-10l3.6 3.6l-10 10H6z"></path></svg>
                        </span>
                        Edit
                    </Button>
                </div>
                <div class="grid gap-y-2 self-end relative z-0 p-4">
                    <div class="absolute bottom-0 left-0 w-100% h-135% -z-1" :class="[$style['gradient--bottom']]"></div>
                    <span class="text-white text-shadow-sm font-semibold text-lg" style="--un-text-shadow-color: black">{{ title }}</span>
                    <ul class="flex flex-wrap gap-x-2 gap-y-1" v-if="tags">
                        <li v-for="{ id, name, color } in tags" :key="id" :style="{
                            'color': color.foreground,
                            'backgroundColor': color.background,
                        }" class="text-xs py-1 px-1.5 leading-tight rounded">{{ name }}</li>
                    </ul>
                </div>
            </div>
        </Transition>
        ...
        >
            <div class="col-[1/-1] row-[1/-1] grid place-items-center relative z-1" v-if="copied">
                <button type="button" @click="handleCopyReset" class="px-3 py-1.5 rounded text-white shadow-sm" :class="
                    [
